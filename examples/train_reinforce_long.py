"""
REINFORCE training script for CivRealm mini-games.
...
def train_reinforce(
    ...
    checkpoint_dir: str = "experiments/checkpoints_reinforce",
    device: str = "cpu",
    lr: float = 3e-4,
    gamma: float = 0.99,
    ...
    for episode in range(1, num_episodes + 1):
        ...
        if episode % checkpoint_freq == 0:
            ckpt_path = Path(checkpoint_dir) / f"reinforce_ep{episode}.pt"
            agent.save(str(ckpt_path))
            print(f"    ✓ Checkpoint saved: {ckpt_path}")
    
    # Save final model
    final_path = Path(checkpoint_dir) / "reinforce_model.pt"
...
def main():
    ...
    parser.add_argument("--checkpoint-dir", type=str, default="experiments/checkpoints_reinforce")
